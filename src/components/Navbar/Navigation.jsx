...
const Navigation = () => {
  ...
  return (
    ...
    >
      ...
      >
        <motion.div
          className="absolute -inset-2 bg-gradient-to-r from-primary-blue/10 to-purple-500/10 rounded-xl opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500"
        />
        <img src="/Logo Full.svg" alt="Cultunesia Logo" className="w-44 relative z-10 drop-shadow-md" />
      </motion.a>
        ...
        {navLinks.map((link, i) => (
          ...
          >
            <span className="relative">
              {link.label}
              <motion.span 
                className="absolute -bottom-2 left-0 w-0 h-[2px] bg-gradient-to-r from-amber-400 to-yellow-600 group-hover:w-full transition-all duration-500 rounded-full shadow-lg shadow-amber-500/50" 
              />
              <motion.span
                className="absolute -inset-x-3 -inset-y-2 bg-amber-400/5 dark:bg-amber-400/10 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"
              />
            </span>
          </motion.a>
        ))}
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-gray-200/50 dark:border-gray-700/50 shadow-xl">
            <DropdownMenuItem onClick={() => setTheme("light")} className="cursor-pointer hover:bg-gradient-to-r hover:from-amber-50 hover:to-orange-50 dark:hover:from-gray-800 dark:hover:to-gray-800">
              <Sun className="mr-3 h-4 w-4 text-amber-500" />
              <span className="font-medium">Light</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTheme("dark")} className="cursor-pointer hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 dark:hover:from-gray-800 dark:hover:to-gray-800">
              <Moon className="mr-3 h-4 w-4 text-indigo-400" />
              <span className="font-medium">Dark</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTheme("system")} className="cursor-pointer hover:bg-gradient-to-r hover:from-gray-50 hover:to-slate-50 dark:hover:from-gray-800 dark:hover:to-gray-800">
        ...
        <div className="md:hidden">
          <Sheet>
            ...
            <SheetContent side="left" className="w-72 sm:w-80 bg-gradient-to-b from-white/98 to-gray-50/98 dark:from-gray-950/98 dark:to-gray-900/98 backdrop-blur-2xl border-r border-gray-200/50 dark:border-gray-700/50 shadow-2xl">
