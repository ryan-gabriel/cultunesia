...
// --- Success Message Component for Email Confirmation ---
const SuccessMessage = ({ email }) => (
  ...
  >
    ...
    >
      ...
      <div className="relative bg-gradient-to-br from-green-400 to-emerald-500 p-4 rounded-full shadow-lg shadow-green-500/30">
    ...
    >
      <h2 className="text-3xl font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white bg-clip-text text-transparent mb-2">
        Pendaftaran Berhasil! 🎉
      </h2>
      <p className="text-gray-600 dark:text-gray-400 text-sm">
        Selamat datang di keluarga Cultunesia
      </p>
    </motion.div>
    ...
    <motion.div
      ...
      className="bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20 p-6 rounded-2xl border border-amber-200/50 dark:border-amber-800/30 shadow-inner"
    >
      ...
      <div className="bg-white dark:bg-gray-800 px-4 py-3 rounded-xl border border-amber-200 dark:border-amber-800/50 mb-4">
        <p className="text-amber-600 dark:text-amber-400 font-bold break-all">
          {email}
        </p>
      </div>
      ...
      <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 text-left">
        <p className="flex items-start gap-2">
          ...
          <span>Periksa kotak masuk atau folder <strong className="text-gray-900 dark:text-white">spam/junk</strong> Anda</span>
        </p>
        <p className="flex items-start gap-2">
          <span className="text-amber-600 dark:text-amber-400 font-bold mt-0.5">•</span>
          <span>Klik tautan konfirmasi dalam email untuk mengaktifkan akun</span>
        </p>
        <p className="flex items-start gap-2">
          <span className="text-amber-600 dark:text-amber-400 font-bold mt-0.5">•</span>
          <span>Setelah dikonfirmasi, Anda dapat masuk ke akun Anda</span>
        </p>
      </div>
    </motion.div>
    ...
    >
      <Link href="/login" passHref>
        <motion.button
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          className="w-full py-4 px-6 bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 hover:from-amber-600 hover:via-yellow-600 hover:to-amber-700 text-white font-bold rounded-xl shadow-lg shadow-amber-500/30 hover:shadow-xl hover:shadow-amber-500/40 transition-all duration-300 flex items-center justify-center gap-2"
        >
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message.includes("duplicate") 
               ? "Email tidak valid. Email sudah terdaftar." 
               : err.message);
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        >
          ...
          <div className="relative z-10">
            ...
            {isRegistered ? (
              <SuccessMessage email={form.email} />
            ) : (
              <>
                {/* Error Message */}
                {error && (
                  ...
                  >
                    <p className="text-red-700 dark:text-red-400 text-sm text-center font-medium">
                      {error.includes("duplicate") || error.includes("already registered")
                        ? "Email tidak valid. Email atau Username sudah terdaftar."
                        : error}
                    </p>
                  </motion.div>
                )}
